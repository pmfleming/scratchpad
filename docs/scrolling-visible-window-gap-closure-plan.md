...
Implementation complete for this plan. The remaining full-document galley allocation is now the display-snapshot/source-map adapter explicitly allowed by the non-goals; normal paint uses the viewport galley when a valid `ViewportSlice` is available. The only remaining pixel facade is the scroll-container interop boundary (`editor_pixel_offset` / `set_editor_pixel_offset`), backed by `ScrollManager` and layout-aware anchor conversion.
