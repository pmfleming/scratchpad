...
| Settings store and settings app state | Store has one default test; mutators/tab order have some local tests | TOML round trip, malformed TOML, legacy YAML migration, older TOML defaults, settings tab persistence, settings TOML edit application | `tests/settings_store_tests.rs` and command workflow tests |
