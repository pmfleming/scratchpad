...
fn preview_lines_for_view(tab: &crate::app::domain::WorkspaceTab, view_id: ViewId) -> Vec<String> {
    let layout_window = tab.view(view_id).and_then(|view| view.published_viewport());
    let buffer = tab.buffer_for_view(view_id);
    if let (Some(window), Some(buffer)) = (layout_window, buffer) {
        return split::build_preview_lines_for_window(buffer, window);
    }
    buffer
