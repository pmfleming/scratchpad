        ...
        egui::containers::menu::SubMenuButton::from_button(button).ui(ui, |ui| {
            ...
            for (label, icon, action) in [
                ...
                if menu_button(ui, TAB_CONTEXT_SUBMENU_WIDTH, label, Some(icon), true) {
                    match action {
                        TabCloseAction::Others => close_other_slots(app, slot_index),
                        TabCloseAction::After => close_slots_after(app, slot_index),
                        TabCloseAction::Saved => close_saved_slots(app),
                        TabCloseAction::All => close_all_slots(app),
                    }
                    ui.close();
                }
            }
