...
pub const DEFAULT_TAB_DISPLAY: TabDisplayMode = TabDisplayMode::Hidden;
...
impl TabDisplayMode {
    pub const ALL: [Self; 3] = [Self::Hidden, Self::Character, Self::Tablines];

    #[must_use]
    pub fn label(self) -> &'static str {
        match self {
            Self::Hidden => "Don't show",
            Self::Character => "Show character",
            Self::Tablines => "Show tablines",
        }
    }
}

impl<'de> Deserialize<'de> for TabDisplayMode {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        struct TabDisplayVisitor;

        impl serde::de::Visitor<'_> for TabDisplayVisitor {
            type Value = TabDisplayMode;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("a tab display mode or legacy boolean")
            }

            fn visit_bool<E>(self, visible: bool) -> Result<Self::Value, E>
            where
                E: serde::de::Error,
            {
                Ok(if visible {
                    TabDisplayMode::Character
                } else {
            ...
            {
                match value {
                    "hidden" => Ok(TabDisplayMode::Hidden),
                    "character" => Ok(TabDisplayMode::Character),
                    "tablines" => Ok(TabDisplayMode::Tablines),
                    _ => Err(E::unknown_variant(
                        value,
                        &["hidden", "character", "tablines"],
                    )),
                }
            }
        }

        deserializer.deserialize_any(TabDisplayVisitor)
    }
}
...
pub struct EditorSettings {
    ...
    #[serde(default, alias = "show_tab_characters")]
