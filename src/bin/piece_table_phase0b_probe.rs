...
struct ComparisonEvent {
    ...
    piece_table_elapsed_ratio_vs_rope: f64,
    piece_table_allocated_ratio_vs_rope: Option<f64>,
    piece_table_peak_ratio_vs_rope: Option<f64>,
    piece_table_elapsed_ratio_vs_string: f64,
    piece_table_allocated_ratio_vs_string: Option<f64>,
    piece_table_peak_ratio_vs_string: Option<f64>,
    piece_table_wins_time_vs_rope: bool,
    piece_table_wins_allocations_vs_rope: bool,
    piece_table_wins_peak_memory_vs_rope: bool,
}

#[derive(Serialize)]
struct DecisionEvent {
    event: &'static str,
    chosen_piece_table_probe_impl: &'static str,
    compared_rope_impl: &'static str,
    compared_string_impl: &'static str,
    comparison_count: usize,
    undo_history_piece_table_beats_rope: bool,
    preview_workloads_favor_rope: bool,
    line_lookup_workloads_favor_rope: bool,
    piece_table_time_wins_vs_rope: usize,
    piece_table_allocation_wins_vs_rope: usize,
    piece_table_peak_wins_vs_rope: usize,
    summary: &'static str,
}
...
impl PieceTableLite {
    fn from_string(text: String) -> Self {
        ...
        let pieces = if text.is_empty() {
            ...
            vec![Piece {
                ...
                start: 0,
                len: len_bytes,
                newline_count: count_newlines(text.as_bytes()),
            }]
        };
        Self {
            original: text,
    ...
    fn insert(&mut self, offset: usize, text: &str) {
        ...
        let (piece_index, inner_offset) = self.find_piece(offset);
        if piece_index == self.pieces.len() {
            self.pieces.push(inserted_piece);
            self.len_bytes += text.len();
            self.normalize_pieces();
            return;
        }

        let existing = self.pieces[piece_index].clone();
        let mut replacement = Vec::with_capacity(3);
        if inner_offset > 0 {
            replacement.push(self.slice_piece(&existing, 0, inner_offset));
        }
        replacement.push(inserted_piece);
        if inner_offset < existing.len {
            replacement.push(self.slice_piece(
                &existing,
                inner_offset,
                existing.len - inner_offset,
            ));
        }
    ...
    fn remove_range(&mut self, range: Range<usize>) {
        ...
        for piece in &self.pieces {
            ...
            if range.end <= piece_start || range.start >= piece_end {
                rebuilt.push(piece.clone());
            } else {
                let overlap_start = range.start.max(piece_start);
                let overlap_end = range.end.min(piece_end);

                let left_len = overlap_start.saturating_sub(piece_start);
                if left_len > 0 {
                    rebuilt.push(self.slice_piece(piece, 0, left_len));
                }

                let right_start = overlap_end.saturating_sub(piece_start);
                if right_start < piece.len {
                    rebuilt.push(self.slice_piece(piece, right_start, piece.len - right_start));
                }
            }
        ...
        self.pieces = rebuilt;
        self.len_bytes -= range.end - range.start;
        self.normalize_pieces();
    }

    fn preview_for_match(&self, range: &Range<usize>) -> (usize, usize, String) {
        let safe_start = range.start.min(self.len_bytes);
        let safe_end = range.end.min(self.len_bytes);

        let mut line_number = 1usize;
        let mut line_start = 0usize;
        let mut pos = 0usize;
        'outer: for piece in &self.pieces {
            for byte in self.piece_text(piece).as_bytes() {
                if pos >= safe_start {
                    break 'outer;
                }
                if *byte == b'\n' {
        ...
        let mut collect_pos = 0usize;
        let mut collecting = false;
        'collect: for piece in &self.pieces {
            for byte in self.piece_text(piece).as_bytes() {
                if collect_pos == line_start {
                    collecting = true;
                }
                if collecting {
                    if *byte == b'\n' {
                ...
                collect_pos += 1;
                if collect_pos >= safe_end && collecting && *byte == b'\n' {
                    break 'collect;
                }
            }
        }

        let preview = compact_preview(std::str::from_utf8(&line_bytes).expect("ASCII line"));
        (line_number, column_number, preview)
    }

    fn line_lookup(&self, target_line: usize) -> (usize, usize) {
    ...
    fn piece_text(&self, piece: &Piece) -> &str {
        match piece.buffer {
            PieceBuffer::Original => &self.original[piece.start..piece.start + piece.len],
            PieceBuffer::Add => &self.add[piece.start..piece.start + piece.len],
        }
    }
    ...
    fn slice_piece(&self, piece: &Piece, offset: usize, len: usize) -> Piece {
        ...
        let text = match piece.buffer {
            PieceBuffer::Original => &self.original[start..start + len],
            PieceBuffer::Add => &self.add[start..start + len],
        };
        Piece {
            buffer: piece.buffer,
            start,
    ...
    fn normalize_pieces(&mut self) {
        let mut normalized: Vec<Piece> = Vec::with_capacity(self.pieces.len());
...
fn emit_load_sweep(comparisons: &mut Vec<ComparisonEvent>) {
    let root = unique_probe_root("piece-table-load");
    std::fs::create_dir_all(&root).expect("create piece-table probe root");

    for (step_index, bytes) in LOAD_SIZES.into_iter().enumerate() {
        let path = root.join(format!("piece_table_probe_load_{bytes}.txt"));
        write_plain_text_file(&path, bytes).expect("write piece-table probe file");
        ...
        let piece_table_measurement = measure("piece_table", || run_piece_table_load(&path));
        emit_measurement_triplet(
            &workload,
            &string_measurement,
            &rope_measurement,
            &piece_table_measurement,
            comparisons,
        );
    }
...
fn emit_insert_sweep(comparisons: &mut Vec<ComparisonEvent>) {
    for (step_index, (base_bytes, insert_bytes)) in INSERT_WORKLOADS.into_iter().enumerate() {
        ...
        let mut piece_table = PieceTableLite::from_string(base_text);
        let piece_table_measurement = measure("piece_table", || {
            piece_table.insert(midpoint, inserted.as_str());
            black_box(piece_table.len_bytes())
        });
...
fn emit_preview_sweep(comparisons: &mut Vec<ComparisonEvent>) {
    for (step_index, bytes) in PREVIEW_SIZES.into_iter().enumerate() {
        ...
        let piece_table = PieceTableLite::from_string(corpus.text.clone());
        ...
        let piece_table_measurement = measure("piece_table", || {
            let (line, column, preview) = piece_table.preview_for_match(&corpus.match_range);
            black_box(line + column + preview.len())
        });
...
fn emit_line_lookup_sweep(comparisons: &mut Vec<ComparisonEvent>) {
    for (step_index, bytes) in LINE_LOOKUP_SIZES.into_iter().enumerate() {
        ...
        let piece_table_measurement = measure("piece_table", || {
            let (line_start, line_len) = piece_table.line_lookup(line_index);
            black_box(line_start + line_len)
        });
...
fn emit_undo_history_sweep(comparisons: &mut Vec<ComparisonEvent>) {
        ...
        UNDO_HISTORY_WORKLOADS.into_iter().enumerate()
    {
        ...
        let piece_table_measurement = measure("piece_table", || {
            run_piece_table_undo_history(base_bytes, edit_count, insert_bytes)
        });
...
fn emit_measurement_triplet(
    ...
    piece_table_measurement: &Measurement,
    ...
    emit_measurement_event(workload, piece_table_measurement);
    ...
    let comparison = ComparisonEvent {
        ...
        piece_table_elapsed_ratio_vs_rope: ratio_u128(
            piece_table_measurement.elapsed_ns,
            rope_measurement.elapsed_ns,
        ),
        piece_table_allocated_ratio_vs_rope: ratio_u64(
            piece_table_measurement.metrics.allocated_bytes,
            rope_measurement.metrics.allocated_bytes,
        ),
        piece_table_peak_ratio_vs_rope: ratio_u64(
            piece_table_measurement.metrics.peak_live_bytes,
            rope_measurement.metrics.peak_live_bytes,
        ),
        piece_table_elapsed_ratio_vs_string: ratio_u128(
            piece_table_measurement.elapsed_ns,
            string_measurement.elapsed_ns,
        ),
        piece_table_allocated_ratio_vs_string: ratio_u64(
            piece_table_measurement.metrics.allocated_bytes,
            string_measurement.metrics.allocated_bytes,
        ),
        piece_table_peak_ratio_vs_string: ratio_u64(
            piece_table_measurement.metrics.peak_live_bytes,
            string_measurement.metrics.peak_live_bytes,
        ),
        piece_table_wins_time_vs_rope: piece_table_measurement.elapsed_ns
            < rope_measurement.elapsed_ns,
        piece_table_wins_allocations_vs_rope: piece_table_measurement.metrics.allocated_bytes
            < rope_measurement.metrics.allocated_bytes,
        piece_table_wins_peak_memory_vs_rope: piece_table_measurement.metrics.peak_live_bytes
            < rope_measurement.metrics.peak_live_bytes,
    };

    println!(
        "{}",
        serde_json::to_string(&comparison).expect("serialize phase0b comparison event")
    );
...
fn emit_measurement_event(workload: &WorkloadDescriptor, measurement: &Measurement) {
    let event = MeasurementEvent {
        ...
        storage_impl: match measurement.storage {
            ...
            "rope" => "ropey",
            "piece_table" => "phase0b::PieceTableLite(Vec<Piece>)",
            _ => "unknown",
        },
        primary_workload_value: workload.primary_value,
        primary_workload_unit: workload.primary_unit,
    ...
    println!(
        "{}",
        serde_json::to_string(&event).expect("serialize phase0b measurement event")
    );
    let _ = std::io::stdout().flush();
}

fn emit_decision_summary(comparisons: &[ComparisonEvent]) {
    let piece_table_time_wins_vs_rope = comparisons
        .iter()
        .filter(|event| event.piece_table_wins_time_vs_rope)
        .count();
    let piece_table_allocation_wins_vs_rope = comparisons
        .iter()
        .filter(|event| event.piece_table_wins_allocations_vs_rope)
        .count();
    let piece_table_peak_wins_vs_rope = comparisons
        .iter()
        .filter(|event| event.piece_table_wins_peak_memory_vs_rope)
        .count();
    let undo_history_piece_table_beats_rope = comparisons
        .iter()
        .filter(|event| event.workload_family == "undo-history")
        .all(|event| event.piece_table_wins_time_vs_rope);
    ...
    let summary = if undo_history_piece_table_beats_rope
        && preview_workloads_favor_rope
        && line_lookup_workloads_favor_rope
    {
        "piece-table-lite improves undo-heavy edit history, but rope still dominates the read-heavy preview and line-lookup workloads"
    } else if piece_table_time_wins_vs_rope >= comparisons.len() / 2 {
        "piece-table-lite is competitive enough with rope to justify a deeper indexed piece-tree prototype"
    } else {
        "piece-table-lite does not displace rope on the combined workload mix in this Phase 0b probe"
    };

    let event = DecisionEvent {
        event: "decision",
        chosen_piece_table_probe_impl: "phase0b::PieceTableLite(Vec<Piece>)",
        compared_rope_impl: "ropey",
        compared_string_impl: "std::string::String",
        comparison_count: comparisons.len(),
        undo_history_piece_table_beats_rope,
        preview_workloads_favor_rope,
        line_lookup_workloads_favor_rope,
        piece_table_time_wins_vs_rope,
        piece_table_allocation_wins_vs_rope,
        piece_table_peak_wins_vs_rope,
        summary,
    };

    println!(
        "{}",
        serde_json::to_string(&event).expect("serialize phase0b decision event")
    );
    let _ = std::io::stdout().flush();
}
...
fn run_piece_table_load(path: &Path) -> usize {
    let text = std::fs::read_to_string(path).expect("read piece-table load file");
    let piece_table = PieceTableLite::from_string(text);
    black_box(piece_table.len_bytes())
}
...
fn run_piece_table_undo_history(
    base_bytes: usize,
    edit_count: usize,
    insert_bytes: usize,
) -> usize {
    let mut piece_table = PieceTableLite::from_string(plain_text_of_size(base_bytes));
    let inserted = plain_text_of_size(insert_bytes);
    let mut undo_log = Vec::with_capacity(edit_count);
    let mut seed = 0xC0FFEE_u64;
    ...
    for _ in 0..edit_count {
        seed = next_seed(seed);
        let offset = (seed as usize) % (piece_table.len_bytes() + 1);
        piece_table.insert(offset, inserted.as_str());
        undo_log.push((offset, inserted.len()));
    }

    while let Some((offset, len)) = undo_log.pop() {
        piece_table.remove_range(offset..offset + len);
    }
...
fn unique_probe_root(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "scratchpad-piece-table-phase0b-{label}-{}-{nanos}",
        std::process::id()
    ))
}
